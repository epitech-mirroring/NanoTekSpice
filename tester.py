...
def print_result(test, result, return_code, print_details):
    ...
    if return_code == 139:
        print(f"Test \"{test['name']}\" failed: the program crashed with signal {return_code}")
        return State.CRASH
    if result != expected_output:
        print(f"Test \"{test['name']}\" failed: expected output is \n{expected_output}\nactual output is \n{result}\n")
        return State.KO
    else:
        if return_code != expected_return_code:
            print(f"Test \"{test['name']}\" failed: expected return code is {expected_return_code}, actual return code is {return_code}")
            return State.KO
        else:
            if print_details:
...
def generate_file(result):
    ...
    for i, test in enumerate(tests):
        test_cases.append(TestCase(test["name"], classname="nanotekspice", stdout=result[i].name, stderr="", elapsed_sec=0.0, status=result[i].name))
